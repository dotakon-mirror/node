...
pub fn decompose_scalar_trits<const N: usize>(value: Scalar) -> [Scalar; N] {
    decompose_trits::<N>(utils::pallas_scalar_to_u256(value))
}
...
mod tests {
    ...
    fn test_div3() {
        assert_eq!(
            div3(utils::parse_pallas_scalar(
                "0x0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20"
            )),
            utils::parse_pallas_scalar(
                "0x005601015702025803035904045a05055b06065c07075d08085e09095f0a0a60"
            )
        );
    }
    ...
    fn test_mod3_large() {
        assert_eq!(
            mod3(utils::parse_pallas_scalar(
                "0x0102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f20"
            )),
            0.into()
        );
